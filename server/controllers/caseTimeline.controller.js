...
exports.getTimelinesByCase = async (req, res) => {
    try {
        ...
        console.log("Fetching timelines for case ID:", caseId);
...
exports.getSingleTimelineEntry = async (req, res) => {
    try {
        ...
        const timeline = await CaseTimeline.findById(timelineId).populate(
            "createdBy",
            "name",
        );
        ...
        if (!timeline) {
            return res
                .status(404)
                .json({ message: "Timeline entry not found" });
        }
...
exports.updateTimelineEntry = async (req, res) => {
    try {
        ...
        const updated = await CaseTimeline.findByIdAndUpdate(
            timelineId,
            updates,
            {
                new: true,
